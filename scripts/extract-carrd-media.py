...
LOCAL_HTML = ROOT / "Bobbio-Russian carrd.html"
LOCAL_ASSET_DIR = ROOT / "Bobbio-Russian carrd_files"
SECTIONS = {
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for key in ("srcset", "data-srcset"):
            for candidate in data.get(key, "").split(","):
                ...
                value = parts[0] if len(parts) == 2 and re.fullmatch(r"\d+(?:\.\d+)?[wx]", parts[1]) else candidate
                self.add(value, context)
...
def local_asset(url: str) -> Path | None:
    """Resolve browser-saved Carrd URLs against the uploaded companion folder."""
    if not LOCAL_ASSET_DIR.is_dir(): return None
    name = Path(unquote(urlparse(url).path)).name
    if not name: return None
    direct = LOCAL_ASSET_DIR / name
    if direct.is_file(): return direct
    matches = [path for path in LOCAL_ASSET_DIR.rglob(name) if path.is_file()]
    return matches[0] if len(matches) == 1 else None


def fetch_asset(url: str) -> tuple[bytes, str, str]:
    local = local_asset(url)
    if local:
        content_type = mimetypes.guess_type(local.name)[0] or "application/octet-stream"
        return local.read_bytes(), content_type, local.name
    data, content_type = fetch(url)
    return data, content_type, Path(urlparse(url).path).name
...
def main() -> int:
    try:
        ...
        elif LOCAL_HTML.is_file(): page = LOCAL_HTML.read_bytes(); content_type = "text/html"
    ...
    # Audit uploaded companion files even when a browser-saved HTML reference was
    # rewritten or omitted. These stay unclassified unless their path is explicit.
    if LOCAL_ASSET_DIR.is_dir():
        referenced_names = {Path(unquote(urlparse(url).path)).name for url, _ in parser.urls}
        for path in LOCAL_ASSET_DIR.rglob("*"):
            if path.is_file() and path.suffix.lower() in IMAGE_EXTENSIONS and path.name not in referenced_names:
                parser.urls.append((urljoin(PAGE_URL, path.name), path.relative_to(LOCAL_ASSET_DIR).as_posix()))
    ...
    for url, context in parser.urls:
        ...
        seen_urls.add(url)
        try: data, content_type, original_name = fetch_asset(url)
        ...
        asset = Asset(url, target.relative_to(ROOT).as_posix(), original_name, filename, section, kind, width, height, len(data), True, True, confidence, "Project media candidate" if section != "unclassified" else "Manual classification required", digest)
